...
const Model = require("../models/crops.model");
const createError = require("http-errors");
const mongoose = require("mongoose");
const ModelName = "Crop";
const {
  uploadImage,
...
module.exports = {
 ...
 delete : async (req, res, next) => {
    try {
        ...
        if (result.nModified === 0) {
            throw createError.NotFound("Crop not found or already deleted");
        }
 ...
 get: async (req, res, next) => {
    try {
        ...
        if (!result) {
            throw createError.NotFound("No Crop Found");
        }
